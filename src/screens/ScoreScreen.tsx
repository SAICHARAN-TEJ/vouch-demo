    ...
    <div className="flex flex-col">
      ...
      <div className="flex flex-col gap-5 p-4 pb-8">
        ...
        <Card glow className="flex flex-col items-center py-6">
          <ProgressRing value={score} size={168} stroke={13}>
            <div className="text-center">
              <div className="tnum text-5xl font-extrabold text-content">{score}</div>
        ...
        {/* Breakdown */}
        <div>
          <h2 className="mb-2 text-sm font-bold text-content">How it's calculated</h2>
          <Card padded={false} className="divide-y divide-border/60">
            <Row label="Base score" value={BASE_SCORE} muted />
            {factors.map((f) => (
              <Row key={f.key} label={f.label} value={f.delta} delta />
            ))}
            <div className="flex items-center justify-between px-4 py-3.5">
              <span className="font-bold text-content">Total</span>
              <span className="tnum text-lg font-extrabold text-primary">{score}</span>
            </div>
          </Card>
        </div>
