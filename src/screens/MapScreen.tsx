...
export function MapScreen() {
  ...
  return (
    <div className="flex flex-col">
      ...
      <div className="p-4">
        {/* Map */}
        <div className="relative h-[300px] overflow-hidden rounded-2xl hairline">
          <VouchMap roadEvents={roadEvents} onSelect={open} className="absolute inset-0" />
          <div className="pointer-events-none absolute bottom-0 left-0 right-0 flex flex-wrap gap-x-3 gap-y-1.5 bg-gradient-to-t from-bg/90 to-transparent p-3">
            {LEGEND.map((t) => (
              <span key={t} className="inline-flex items-center gap-1.5 text-[11px] font-medium text-content">
                <span className={`h-2 w-2 rounded-full ${hazardBg(t)}`} />
                {ROAD_EVENT_LABEL[t]}
              </span>
            ))}
          </div>
        ...
        <div className="mt-5 flex items-center justify-between">
          <h2 className="text-sm font-bold text-content">Reported hazards</h2>
        ...
        {error && (
          <div role="alert" className="mt-3 rounded-xl bg-danger/10 p-3 text-xs text-danger ring-1 ring-inset ring-danger/25">
