...
export function StatTile({
  ...
  return (
    <div className={cn("card p-3.5", className)}>
      {icon && (
        <div className={cn("mb-2 inline-grid h-8 w-8 place-items-center rounded-lg bg-white/5", accent)}>
          <Icon name={icon} className="h-4 w-4" />
        </div>
      )}
      <div className="tnum text-2xl font-bold leading-none text-content">{value}</div>
